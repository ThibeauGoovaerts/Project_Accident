...
function clickItem(parentID, clicked_id, startZero) {
    // We get all the divs from the page with the details in an array
    let details = document.getElementsByClassName("childElAgentMaterial");
    ...
    if (!document.getElementById('showAllParentsAgent').checked) {
        SearchBarInAddAgentMateriel('listAgentsMat', 'searchBarAgents')
    }
 ...
 */
function SearchBarInAddAgentMateriel(idList, idSearchBar) {
    ...
    let detail = $("#" + 'listAgentsMatDetail'),
    ...
    for (i = 0; i < listDetails.length; i++) {
        ...
        if (filter === "") {
            ...
            if (txtValue.toUpperCase().indexOf(filter) > -1) {
                ...
                document.getElementsByClassName('agentID' + id_to_compare)[0].style.display = "";
            } else {
 ...
 * */
function clickReactDetail(categorieID, detailID) {
...
/* 
    This eventlistener will listen to all keyboard inputs on the document and will insert them (when focused on the material agent page) into our searchbar
 */
document.body.addEventListener("keyup", function (event) {
    // The if statement to verify that the correct tab is in view
    if (document.getElementById('tabAgentMateriel').style.display === "block") {
        // if the input is a ascii caracter, we add it to the searchbar
        if (event.keyCode > 48 && event.keyCode < 90) {
            document.getElementById('searchBarAgents').value += String.fromCharCode(event.keyCode).toLowerCase();
        } else if (event.keyCode == 8) { // if the caracter is a backspace, we artificially backspace the searchbar
            document.getElementById('searchBarAgents').value = document.getElementById('searchBarAgents').value.slice(0, -1);
        } else if (event.keyCode == 32) { // if the caracter is a space, we artificially add a space
            if (!(document.activeElement.tagName === "INPUT" && document.activeElement.type === "checkbox"))
                document.getElementById('searchBarAgents').value += " ";
        }
        // We call the function for the searchbar
        SearchBarInAddAgentMateriel('listAgentsMat', 'searchBarAgents')
    } else if (document.getElementById('tabDeviation').style.display === "block") {
        // if the input is a ascii caracter, we add it to the searchbar
        if (event.keyCode > 48 && event.keyCode < 90) {
            document.getElementById('searchBarDeviation').value += String.fromCharCode(event.keyCode).toLowerCase();
        } else if (event.keyCode == 8) { // if the caracter is a backspace, we artificially backspace the searchbar
            document.getElementById('searchBarDeviation').value = document.getElementById('searchBarDeviation').value.slice(0, -1);
        } else if (event.keyCode == 32) { // if the caracter is a space, we artificially add a space
            if (!(document.activeElement.tagName === "INPUT" && document.activeElement.type === "checkbox"))
                document.getElementById('searchBarDeviation').value += " ";
        }
        // We call the function for the searchbar
        SearchBarInAddDeviation('listDeviations', 'searchBarDeviation')
    }
});
 ...
 */
document.getElementById('showAllParentsAgent').addEventListener('click', () => {

    // the slider element
    el = document.getElementById('showAllParentsAgent')

    //All the categories
    child_els = $("#" + 'listAgentsMat').children();
